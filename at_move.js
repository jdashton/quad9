...
Elm.AtMove.make = function (_elm) {
   ...
   var keyboardStep = F2(function (newKeys,state) {
      ...
      newKeys,
      state.temp)));
      return _U.update(state,{temp: newKeys,actualKey: actualKey});
   });
   var keyboardState = {temp: $Set.empty
                       ,actualKey: $Maybe.Nothing};
   var presses = A2($Signal.map,
   function (state) {
      return state.actualKey;
   },
   A3($Signal.foldp,
   keyboardStep,
   keyboardState,
   $Keyboard.keysDown));
   var KeyboardState = F2(function (a,b) {
      return {temp: a,actualKey: b};
   });
   var stepPlayer = F2(function (key,player) {
      var _p0 = key;
      switch (_p0)
      {case 37: return _U.update(player,
           {x: player.x - 1,y: player.y + 0});
         case 38: return _U.update(player,
           {x: player.x + 0,y: player.y - 1});
         case 39: return _U.update(player,
           {x: player.x + 1,y: player.y + 0});
         case 40: return _U.update(player,
           {x: player.x + 0,y: player.y + 1});
   ...
   var move = F3(function (_p2,_p1,form) {
      var _p3 = _p2;
      var _p4 = _p1;
      var screenSize$ = $Basics.toFloat(screenSize);
      var y$ = $Basics.toFloat(_p4._1);
      var x$ = $Basics.toFloat(_p4._0);
   ...
   var view = F2(function (_p6,_p5) {
      var _p7 = _p6;
      var _p10 = _p7._0;
      var _p9 = _p7._1;
      var _p8 = _p5;
      var s = A2($Basics.min,_p10,_p9);
