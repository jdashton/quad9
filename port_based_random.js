...
Elm.PortBasedRandom = Elm.PortBasedRandom || {};
Elm.PortBasedRandom.make = function (_elm) {
   "use strict";
   _elm.PortBasedRandom = _elm.PortBasedRandom || {};
   if (_elm.PortBasedRandom.values) return _elm.PortBasedRandom.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   ...
   var _op = {};
   var randomInt = function (model) {
      var _p0 = A2($Random.generate,A2($Random.$int,1,10),model.nextSeed);
      var i = _p0._0;
      var s = _p0._1;
      return _U.update(model,{nextSeed: s,currentInt: i});
   };
   var update = F2(function (_p1,model) {    var _p2 = _p1;return randomInt(model);});
   var Model = F2(function (a,b) {    return {nextSeed: a,currentInt: b};});
   var primer = Elm.Native.Port.make(_elm).inbound("primer","Float",function (v) {    return typeof v === "number" ? v : _U.badPort("a number",v);});
   var firstSeed = $Random.initialSeed($Basics.round(primer));
   var initialModel = {nextSeed: firstSeed,currentInt: 0};
   var main = A2($Signal.map,function (m) {    return $Graphics$Element.show(m.currentInt);},A3($Signal.foldp,update,initialModel,$Mouse.position));
   return _elm.PortBasedRandom.values = {_op: _op,firstSeed: firstSeed,Model: Model,initialModel: initialModel,randomInt: randomInt,update: update,main: main};
};
